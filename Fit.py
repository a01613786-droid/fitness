...
def user_input_features():
  ...
  activity_index = st.number('activity_index', min_value=0, max_value=5, value = 0, step = 1)
  smokes = st.number('smokes', min_value=0, max_value=1, value = 0, step = 1)
  gender = st.number('gender', min_value=0, max_value=1, value = 0, step = 1)
...
fit =  pd.read_csv('Fitness_Classification2.csv', encoding='latin-1')
X = fit.drop(columns='Survived')
Y = fit['Survived']
